...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        rows = 21
        cols = 3
        Lmlist = [[0 for j in range(cols)] for i in range(rows)]
        xList = []
        yList = []
        fingers=[]
            ...
            if (results.right_hand_landmarks is None or results.left_hand_landmarks is None) and (
                ...
                if results.right_hand_landmarks is not None:
                    ...
                    for j, lm in enumerate(results.right_hand_landmarks.landmark):
                        Lmlist[j][0] = round(lm.x * 1280)
                        Lmlist[j][1] = round(lm.y * 1280)
                        Lmlist[j][2] = round(lm.z * 1280)
                        xList.append(round(lm.x * 1280))
                        yList.append(round(lm.y * 1280))
